...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    int W_unroll = H_out * W_out;
    float* X_unroll;
    cudaMalloc(&X_unroll, sizeof(float)* W_unroll * H_unroll * num_pic);

    int tile_size2 = TILE_WIDTH2;

    dim3 gridDim2 (ceil((1.*C*H_out*W_out) / tile_size2), 1, 1);
    dim3 blockDim2 (tile_size2, num_pic, 1);

    dim3 gridDim1 (ceil(1.0 * W_unroll * num_pic/ TILE_WIDTH1),  ceil(1.0 * M/ TILE_WIDTH1), 1);
    dim3 blockDim1 (TILE_WIDTH1, TILE_WIDTH1, 1);

    for (int b = 0; b < B; b += num_pic) {
        unroll_kernel<<<gridDim2, blockDim2>>>(C, H, W, K, device_x+b*C*H*W, X_unroll, num_pic);
        matrixMultiplyShared<<<gridDim1, blockDim1>>>(device_k, X_unroll, device_y + b*M*W_unroll, M, H_unroll, H_unroll, W_unroll*num_pic,M, W_unroll*num_pic,W_unroll);     // H_unroll is the height of X, M is the width of X
    }
    cudaFree(X_unroll);
}
