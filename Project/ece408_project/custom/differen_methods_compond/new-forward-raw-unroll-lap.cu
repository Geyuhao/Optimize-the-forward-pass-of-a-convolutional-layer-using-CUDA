...
__host__ void GPUInterface::conv_forward_gpu_prolog(float *host_y, const float *host_x, const float *host_k, float **device_y_ptr, float **device_x_ptr, float **device_k_ptr, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    cudaMalloc(&X_unroll, parellel*sizeof(float)* W_unroll * H_unroll);
    ...
    for (int b = 0; b < B; b+=parellel) {
        cudaMemcpyAsync(*device_x_ptr + (b+0)*C*H*W, host_x + (b+0)*C*H*W,sizeof(float)*C*H*W,cudaMemcpyHostToDevice, stream0);
        cudaMemcpyAsync(*device_x_ptr + (b+1)*C*H*W, host_x + (b+1)*C*H*W,sizeof(float)*C*H*W,cudaMemcpyHostToDevice, stream1);
    
        unroll_kernel<<<gridDim2, blockDim2, 0, stream0>>>(C, H, W, K, *device_x_ptr+(b+0)*C*H*W, X_unroll+(0*W_unroll * H_unroll));
        unroll_kernel<<<gridDim2, blockDim2, 0, stream1>>>(C, H, W, K, *device_x_ptr+(b+1)*C*H*W, X_unroll+(1*W_unroll * H_unroll));

        matrixMultiplyShared<<<gridDim1, blockDim1, 0, stream0>>>(*device_k_ptr, X_unroll+(0*W_unroll * H_unroll), *device_y_ptr + (b+0)*M*W_unroll, M, H_unroll, H_unroll, W_unroll,M, W_unroll);     // H_unroll is the height of X, M is the width of X
        matrixMultiplyShared<<<gridDim1, blockDim1, 0, stream1>>>(*device_k_ptr, X_unroll+(1*W_unroll * H_unroll), *device_y_ptr + (b+1)*M*W_unroll, M, H_unroll, H_unroll, W_unroll,M, W_unroll);     // H_unroll is the height of X, M is the width of X


        cudaMemcpyAsync(host_y + (b+0)*M*W_unroll, *device_y_ptr + (b+0)*M*W_unroll, sizeof(float)*M*H_out*W_out,cudaMemcpyDeviceToHost, stream0);
        cudaMemcpyAsync(host_y + (b+1)*M*W_unroll, *device_y_ptr + (b+1)*M*W_unroll, sizeof(float)*M*H_out*W_out,cudaMemcpyDeviceToHost, stream1);
    }
    cudaFree(X_unroll);
