...
__host__ void GPUInterface::conv_forward_gpu_prolog(float *host_y, const float *host_x, const float *host_k, float **device_y_ptr, float **device_x_ptr, float **device_k_ptr, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    for (int i=0; i<times; i++)
    {
        ...
        conv_forward_kernel_shared_constant_multikernel<<<gridDim_choice,blockDim_choice,shemem_size_choice,stream0>>>(tile_size, *device_y_ptr + (0+stream_num*i)*small_batch*M*H_out*W_out,*device_x_ptr + (0+stream_num*i)*small_batch*C*H*W,*device_k_ptr,small_batch,M,C,H,W,K);
        conv_forward_kernel_shared_constant_multikernel<<<gridDim_choice,blockDim_choice,shemem_size_choice,stream1>>>(tile_size, *device_y_ptr + (1+stream_num*i)*small_batch*M*H_out*W_out,*device_x_ptr + (1+stream_num*i)*small_batch*C*H*W,*device_k_ptr,small_batch,M,C,H,W,K);
        conv_forward_kernel_shared_constant_multikernel<<<gridDim_choice,blockDim_choice,shemem_size_choice,stream2>>>(tile_size, *device_y_ptr + (2+stream_num*i)*small_batch*M*H_out*W_out,*device_x_ptr + (2+stream_num*i)*small_batch*C*H*W,*device_k_ptr,small_batch,M,C,H,W,K);
        conv_forward_kernel_shared_constant_multikernel<<<gridDim_choice,blockDim_choice,shemem_size_choice,stream3>>>(tile_size, *device_y_ptr + (3+stream_num*i)*small_batch*M*H_out*W_out,*device_x_ptr + (3+stream_num*i)*small_batch*C*H*W,*device_k_ptr,small_batch,M,C,H,W,K);
        conv_forward_kernel_shared_constant_multikernel<<<gridDim_choice,blockDim_choice,shemem_size_choice,stream4>>>(tile_size, *device_y_ptr + (4+stream_num*i)*small_batch*M*H_out*W_out,*device_x_ptr + (4+stream_num*i)*small_batch*C*H*W,*device_k_ptr,small_batch,M,C,H,W,K);
